...
class _LunchWidgetState extends State<LunchWidget> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      if (widget.tab == 0) {
        _model.lunchlist = await queryNutritionsRecordOnce(
        ...
        setState(() {
          _model.snackListname =
              _model.lunchlist!.toList().cast<NutritionsRecord>();
        });
      } else {
        if (widget.tab == 1) {
          _model.lunchlistW = await queryNutritionsRecordOnce(
            parent: FFAppState().userIdRef,
            queryBuilder: (nutritionsRecord) => nutritionsRecord
                .where(
                  'mealtime',
          ...
          setState(() {
            _model.snackListname =
                _model.lunchlistW!.toList().cast<NutritionsRecord>();
          });
        } else {
          _model.lunchlistM = await queryNutritionsRecordOnce(
            parent: FFAppState().userIdRef,
            queryBuilder: (nutritionsRecord) => nutritionsRecord
                .where(
                  'mealtime',
          ...
          setState(() {
            _model.snackListname =
                _model.lunchlistM!.toList().cast<NutritionsRecord>();
          });
        }
      }
    });
